...
export class EditBug extends Component
{
    ...
    getBugById = (id) => {
        axios.get(config.serverUrl + "/api/bug/getbyid/" + id).then(response=> {
            this.setState({
                ...
                reporterId: response.data.reporter.id,
                assigneeId: response.data.assignee.id,
                module: response.data.module,
                platform: response.data.platform,
                version: response.data.version,
                testerId: response.data.tester.id,
    ...
    updateProject = () => {
        ...
        if (isValid)
        {
            ...
            axios.put(config.serverUrl + "/api/bug/update", bug).then(response=> {
                this.props.history.push("/bug-detail/" + this.state.id);
            })
        }
    }
    ...
    cancelUpdate = () => {
        this.props.history.push("/bug-detail/" + this.props.match.params.id);
    }
